...
public class OrderServerInforatmion {
	...
	public static void writeClass() throws IOException{
		s = new Socket("10.62.150.16",8888); //与服务器建立连接，目前运行在本机
		System.out.println("Connected to the server with 10.62.150.16:8888");
		ObjectOutputStream oos = new ObjectOutputStream(s.getOutputStream());
		NodeInfomation ni = new NodeInfomation();
		CpuUsage cpu = new CpuUsage();
		ni.setCpuUsage(cpu.getCpuRatioForWindows());
		ni.setNodeName("10.62.150.16"); //都跑在16上，要根据实际情况调用
		oos.writeObject(ni);
		oos.flush();
		oos.close();
	}
